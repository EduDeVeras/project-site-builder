import { transformadores, logEventos } from '@/lib/mock-data';
...
export default function Dashboard() {
  ...
  const mediaTemp = (transformadores.reduce((s, t) => s + t.temperatura_teste, 0) / totalTransformadores).toFixed(1);
  const recentLogs = logEventos.slice().sort((a, b) => new Date(b.data_hora).getTime() - new Date(a.data_hora).getTime()).slice(0, 5);
            ...
            {transformadores.slice(0, 4).map(t => (
              <div key={t.id} className="flex items-center justify-between py-2 border-b border-border last:border-0">
                ...
                <div className="flex items-center gap-3">
                  <span className="text-xs font-mono text-muted-foreground">{t.temperatura_teste}°C</span>
